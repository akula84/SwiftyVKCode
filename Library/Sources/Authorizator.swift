...
internal struct Authorizator {
    ...
    fileprivate static var paramsUrl: String? {
        ...
        return  "client_id=\(appId)&scope=\(_perm)&display=mobile&v\(VK.config.apiVersion)&sdk_version=\(VK.config.sdkVersion)\(_redir)&response_type=code&revoke=\(Token.revoke ? 1 : 0)"
    }
